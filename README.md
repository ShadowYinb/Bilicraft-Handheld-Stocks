...
- 根据连续 15 分钟剩余股数变化，在 K 线上标注“售出/回购 X 股”。
- 根据当前可见范围显示最高价、最低价及自适应价格/时间刻度。
- K 线区域纵向滑动可自然滚动外层页面。
- 股票列表独立纵向滚动，边界滑动不会带动外层页面。
- 自动维护网页公司名称与 Minecraft 服务器股票 ID 的本地映射。
- 进入股票页面时自动查询玩家资金和持股。
...
- 本地只保存股票公司名称与服务器 ID 的映射文件 `stock_company_ids.json`。
...
```text
build/outputs/bhplugin/stock-market-0.1.7.bhplugin
```
...
```bash
gradle --no-daemon clean packageBhPlugin
unzip -t build/outputs/bhplugin/stock-market-0.1.7.bhplugin
unzip -p build/outputs/bhplugin/stock-market-0.1.7.bhplugin plugin.json
sha256sum build/outputs/bhplugin/stock-market-0.1.7.bhplugin
```
